...
fig <- fig %>% layout(title = paste0("BTCEUR: ", str_sub(start.time, 1, 10), " to ", Sys.Date()),
...
strategy <- df_bt %>%
            ...
            mutate(date = as.Date(OpenTime),
                   fast_sma = SMA(Close, 5),
                   slow_sma = SMA(Close, 20),
                   pos = if_else(fast_sma > slow_sma, 1, -1) %>% lag() %>% replace_na(0),
                   str_pnl = pos*pnl
            )
strategy[1:50, c('OpenTime', 'Close', 'fast_sma', 'slow_sma', 'pos', 'str_pnl')]

# Plot (Close, fast_sma, slow_sma)
plot(strategy$OpenTime, strategy$Close, type="l")
points(strategy$OpenTime, strategy$fast_sma, type="l", col="red", lwd=1.5)
points(strategy$OpenTime, strategy$slow_sma, type="l", col="green", lwd=1.5)
# abline(v=strategy$OpenTime[which(strategy$pos == 1)], lwd=0.01, lty=2)
